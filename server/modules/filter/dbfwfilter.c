...
TIMERANGE* parse_time(char* str, FW_INSTANCE* instance)
{
	...
	if(tr == NULL){
		mxs_log(LOGFILE_ERROR, "dbfwfilter: malloc returned NULL.");
		return NULL;
	}
...
RULE* find_rule(char* tok, FW_INSTANCE* instance)
{
	...
	mxs_log(LOGFILE_ERROR, "Error : Rule not found: %s",tok);	
	return NULL;
}
...
bool link_rules(char* orig, FW_INSTANCE* instance)
{
	...
	if((userptr == NULL || ruleptr == NULL || modeptr == NULL)||
	   (userptr > modeptr || userptr > ruleptr || modeptr > ruleptr)) {
		mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, right keywords not found in the correct order: %s",orig);
		rval = false;
		goto parse_err;
	}
	...
	*ruleptr++ = '\0';
	...
	if(strcmp(tok,"match") == 0){
		...
		if(tok == NULL)
		{
		    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, missing keyword after 'match': %s",orig);
		    rval = false;
		    goto parse_err;
		}
		if(strcmp(tok,"any") == 0){
			...
			mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, 'match' was not followed by correct keyword: %s",orig);
			rval = false;
			goto parse_err;
		}
	}
	...
	{
	    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, bad token: %s",tok);
	    rval = false;
	    goto parse_err;
	}
	...
	if(tok != NULL)
	{
	    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, extra token found after 'match' keyword: %s",orig);
	    rval = false;
	    goto parse_err;
	}
	...
	if(tok == NULL)
	{
	    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, no rules given: %s",orig);
	    rval = false;
	    goto parse_err;
	}
	...
	while(tok)
	{
	    ...
	    {
		mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, could not find rule '%s'.",tok);
		rval = false;
		goto parse_err;
	    }
	    tok = strtok_r(NULL," ",&saveptr);
	...
	*(ruleptr) = '\0';
	...
	if(userptr == NULL)
	{
	    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, no users given: %s",orig);
	    rval = false;
	    goto parse_err;
	}
	...
	if(rulelist == NULL)
	{
	    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule syntax incorrect, no rules found: %s",orig);
	    rval = false;
	    goto parse_err;
	}
	...
	while(userptr)
	{
        ...
        if((user = (USER*)hashtable_fetch(instance->htable,userptr)) == NULL){
            ...
            if(user == NULL){
		mxs_log(LOGFILE_ERROR,"Error: dbfwfilter: failed to allocate memory when parsing rules.");
		rval = false;
		goto parse_err;
            }
...
bool parse_rule(char* rule, FW_INSTANCE* instance)
{
    ...
    if(tok == NULL)
    {
	mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, no rule: %s",rule);
	rval = false;
	goto retblock;
    }
	...
	if(tok == NULL)
	{
	    mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, incomplete rule: %s",rule);
	    rval = false;
	    goto retblock;
	}
    ...
    {
	mxs_log(LOGFILE_ERROR,"Error : Unknown token in rule '%s': %s",rule,tok);
	rval = false;
	goto retblock;
    }
    ...
    if(tok == NULL)
    {
	mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, no allow or deny: %s",rule);
	rval = false;
	goto retblock;
    }
	...
	while(tok)
	{
		...
		if(strcmp(tok,required_rules[i]) == 0)
		{
		    if(req_defined)
		    {
			mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, Multiple non-optional rules: %s",rule);
			rval = false;
			goto retblock;
		    }
		    else
		...
		if(at_def)
		{
		    mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, multiple 'at_times' tokens: %s",rule);
		    rval = false;
		    goto retblock;
		}
		at_def = true;
		    ...
		    if(!check_time(tok))
		    {
			mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, malformed time definition: %s",tok);
			rval = false;
			goto retblock;
		    }
		    ...
		    if(tmp == NULL)
		    {
			mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, unexpected characters after time definition.");
			rval = false;
			tr_free(tr);
			goto retblock;
		    }
		...
		if(tok == NULL)
		{
		    mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, No regex string.");
		    rval = false;
		    goto retblock;
		}
		...
		if(*tok != '\'' && *tok != '\"')
		{
		    mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, regex string not quoted.");
		    rval = false;
		    goto retblock;
		}
		...
		if(n_char >= 2048)
		{
		    mxs_log_flush(LOGFILE_ERROR, "dbfwfilter: Failed to parse rule, regular expression length is over 2048 characters.");
		    rval = false;
                    goto retblock;
		}
                ...
                if(str == NULL)
                {
                    mxs_log_flush(LOGFILE_ERROR, "Fatal Error: malloc returned NULL.");
		    rval = false;
                    goto retblock;
                }
		    ...
		    rval = false;
                ...
                if(regcomp(re, str,REG_NOSUB|instance->regflags)){
                    mxs_log(LOGFILE_ERROR, "dbfwfilter: Invalid regular expression '%s'.", str);
		    rval = false;
                    free(re);
		    goto retblock;
                }
		...
		char *errptr = NULL;
                ...
                if(tok == NULL){
                    free(qs);
		    rval = false;
		    mxs_log(LOGFILE_ERROR, "dbfwfilter: Missing parameter in limit_queries: '%s'.", rule);
                    goto retblock;
                }
		...
		if(errptr && *errptr != '\0')
		{
		    free(qs);
		    rval = false;
		    mxs_log(LOGFILE_ERROR, "dbfwfilter: Rule parsing failed, not a number: '%s'.", tok);
                    goto retblock;
		}
		...
		if(qs->limit < 1){
                    free(qs);
		    rval = false;
		    mxs_log(LOGFILE_ERROR, "dbfwfilter: Bad query amount: %s", tok);
                    goto retblock;
                }
		...
		if(qs->period < 1){
                    free(qs);
		    rval = false;
		    mxs_log(LOGFILE_ERROR, "dbfwfilter: Bad time period: %s", tok);
                    goto retblock;
                }
		...
		if(qs->cooldown < 1){
                    free(qs);
		    rval = false;
		    mxs_log(LOGFILE_ERROR, "dbfwfilter: Bad blocking period: %s", tok);
                    goto retblock;
                }
		...
		if(oq_def)
		{
		    mxs_log(LOGFILE_ERROR,"dbfwfilter: Rule parsing failed, multiple 'on_queries' tokens: %s",rule);
		    rval = false;
		    goto retblock;
		}
		oq_def = true;
...
int parse_rulefile(FW_INSTANCE* my_instance,char* filename)
{
    ...
    if(file_empty)
    {
	mxs_log(LOGFILE_ERROR,"dbfwfilter: File is empty: %s",filename);
	rval = -1;
    }
...
createInstance(char **options, FILTER_PARAMETER **params)
{
	...
	if ((my_instance = calloc(1, sizeof(FW_INSTANCE))) == NULL ||
		(my_instance->lock = (SPINLOCK*)malloc(sizeof(SPINLOCK))) == NULL){
            mxs_log(LOGFILE_ERROR, "Memory allocation for firewall filter failed.");
		return NULL;
	}
	...
	if(filename == NULL)
	{
	    mxs_log(LE,"Error: No 'rules' parameter found.");
	}

	spinlock_init(my_instance->lock);

	if((ht = hashtable_alloc(100, hashkeyfun, hashcmpfun)) == NULL){
		mxs_log(LOGFILE_ERROR, "Unable to allocate hashtable.");
		free(my_instance);
		return NULL;
	}
	...
	if(ptr == NULL)
	{
	    mxs_log(LOGFILE_ERROR,"dbfwfilter: No 'users' line found.");
	    err = true;
	    goto retblock;
	}
	...
	while(ptr){
	    ...
	    {
		mxs_log(LOGFILE_ERROR,"dbfwfilter: Failed to parse rule: %s",ptr->value);
		err = true;
	    }
	    tmp = ptr;
	    ptr = ptr->next;
...
updateInstance(FILTER *instance, char **options, FILTER_PARAMETER **params)
{
    ...
    {
	mxs_log(LE,"Error: Failed to update filter.");
	return -1;
    }
...
GWBUF* gen_dummy_error(FW_SESSION* session, char* msg)
{
	...
	unsigned int errlen;
        ...
        {
            mxs_log_flush(LOGFILE_ERROR, "Error : Firewall filter session missing data.");
            return NULL;
        }
	...
	if(errmsg == NULL){
		mxs_log_flush(LOGFILE_ERROR, "Fatal Error: Memory allocation failed.");	
		return NULL;
	}
...
bool rule_matches(FW_INSTANCE* my_instance, FW_SESSION* my_session, GWBUF *queue, USER* user, RULELIST *rulelist, char* query)
{
	...
	if(rulelist->rule->on_queries == QUERY_OP_UNDEFINED || rulelist->rule->on_queries & optype){
        ...
        case RT_UNDEFINED:
            mxs_log_flush(LOGFILE_ERROR, "Error: Undefined rule type found.");	
            break;
        ...
        case RT_REGEX:
            ...
            if(query && regexec(rulelist->rule->data,query,0,NULL,0) == 0){
                ...
                if(!rulelist->rule->allow){
                    ...
                    mxs_log(LOGFILE_TRACE, "dbfwfilter: rule '%s': regex matched on query",rulelist->rule->name);
                    goto queryresolved;
                }else{
                    break;
                }
        ...
        case RT_PERMISSION:
            if(!rulelist->rule->allow){
                ...
                mxs_log(LOGFILE_TRACE, "dbfwfilter: rule '%s': query denied at: %s",rulelist->rule->name,asctime(tm_now));
                goto queryresolved;
            }else{
                break;
            }
            ...
            if(is_sql && is_real)
	    {
			...
			while(strln)
			{
				...
				if(!rulelist->rule->allow)
				{
				    ...
				    mxs_log(LOGFILE_TRACE, "dbfwfilter: rule '%s': query targets forbidden column: %s",rulelist->rule->name,strln->value);
				    msg = strdup(emsg);
				    goto queryresolved;
				}
				else
        ...
        case RT_WILDCARD:
			...
			mxs_log(LOGFILE_TRACE, "dbfwfilter: rule '%s': query contains a wildcard.",rulelist->rule->name);
			goto queryresolved;
		    }
		    free(where);
		}
		...
		if(difftime(time_now,queryspeed->triggered) < queryspeed->cooldown)
		{
		    ...
		    mxs_log(LOGFILE_TRACE, "dbfwfilter: rule '%s': user denied for %f seconds",rulelist->rule->name,blocked_for);
		    msg = strdup(emsg);
        ...
        case RT_CLAUSE:
               ...
               !skygw_query_has_clause(queue))
            {
                ...
                mxs_log(LOGFILE_TRACE, "dbfwfilter: rule '%s': query has no where/having clause, query is denied.",
                                rulelist->rule->name);
            }
            break;
...
int main(int argc, char** argv)
{
    ...
    mxs_logmanager_init(argc_,argv_);
