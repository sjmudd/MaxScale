        ...
        char*  cnf_file_arg)
{
	...
	char logbuf[PATH_MAX + 256];
		...
		if((*cnf_full_path = (char *)malloc(PATH_MAX+1)) == NULL)
		{
		    skygw_log_write(LE,"[%s] Error: Memory allocation failed when allocating %d bytes.",
			     __FUNCTION__,PATH_MAX+1);
		}
                ...
                if (realpath(cnf_file_arg, *cnf_full_path) != NULL)
                {
                         ...
                         else
                         {
                                 char* logstr = "Failed to open read access to "
                                         "config file '%s'.";
                                 int eno = errno;
				 snprintf(logbuf,PATH_MAX+255,logstr,cnf_file_arg);
