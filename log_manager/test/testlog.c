...
int main(int argc, char* argv[])
{
        ...
        i = atexit(mxs_logmanager_exit);
        
        if (i != 0) {
                fprintf(stderr, "Couldn't register exit function.\n");
        }
        
        succp = mxs_logmanager_init( log_argc, log_argv);
	if(!succp)
        ...
        t = time(NULL);
        tm = *(localtime(&t));
        err = mxs_log_flush(LOGFILE_ERROR,
        ...
        mxs_logmanager_init( log_argc, log_argv);
        logstr = ("First write with flush.");
        err = mxs_log_flush(LOGFILE_ERROR, logstr);

        logstr = ("Second write with flush.");
        err = mxs_log_flush(LOGFILE_ERROR, logstr);

        logstr = ("Third write, no flush.");
        err = mxs_log(LOGFILE_ERROR, logstr);

        logstr = ("Fourth write, no flush. Next flush only.");
        err = mxs_log(LOGFILE_ERROR, logstr);

        err = mxs_flush_file(LOGFILE_ERROR);        
        
        logstr = "My name is %s %d years and %d months.";
#if !defined(SS_DEBUG)
        mxs_log_enable(LOGFILE_TRACE);
#endif
        err = mxs_log(LOGFILE_TRACE, logstr, "TraceyTracey", 3, 7);
        mxs_flush_file(LOGFILE_TRACE);
#if !defined(SS_DEBUG)
        mxs_log_enable(LOGFILE_TRACE);
#endif
        logstr = "My name is Tracey Tracey 47 years and 7 months.";
        err = mxs_log(LOGFILE_TRACE, logstr);
#if !defined(SS_DEBUG)
        mxs_log_enable(LOGFILE_TRACE);
#endif        
        logstr = "My name is Stacey %s";
        err = mxs_log_flush(LOGFILE_TRACE, logstr, "           ");
        mxs_logmanager_done();
#if !defined(SS_DEBUG)
        mxs_log_enable(LOGFILE_TRACE);
#endif
...
#endif
        logstr = "Ph%dlip.";
        err = mxs_log(LOGFILE_TRACE, logstr, 1);
        ...
        err = mxs_log(LOGFILE_MESSAGE, logstr);
        ...
        mxs_log_enable(LOGFILE_ERROR);
        mxs_log_enable(LOGFILE_MESSAGE);
        ...
        err = mxs_log_flush(LOGFILE_MESSAGE, logstr);
        ss_dassert(err == 0);
        ...
        err = mxs_log(LOGFILE_ERROR,
                              "11.\tWrite to all logs some formattings : "
                              "%d %s %d",
                              (int)3,
                              "foo",
                              (int)3);
        err = mxs_log(LOGFILE_MESSAGE,
        ...
        void* data)
{
        ...
        for (i=0; i<NITER; i++) {
                ...
                err = mxs_log((logfile_id_t)(rand()%(LOGFILE_LAST+1)),
                                      "%s - iteration # %d",
                                      str,
                                      i);
		if(err != 0)
